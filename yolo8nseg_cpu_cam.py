...
def main():
    ...
    ap.add_argument("--imgsz", type=int, default=416)  # CPU için 416/320 daha mantıklı
    ap.add_argument("--conf", type=float, default=0.25)
    ap.add_argument("--draw", action="store_true", help="mask/box çiz (FPS düşürür)")
    args = ap.parse_args()
    ...
    if not cap.isOpened():
        print("[ERROR] Camera open failed via CAP_GSTREAMER.")
        return
    ...
    print("[INFO] CPU mode running. Keys: q=quit, d=toggle draw")
    while True:
        ok, frame = cap.read()
        ...
        cv2.imshow("YOLOv8n-seg CPU (Jetson)", out)
